...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        Classifier model = loadModel("models/decision_tree_model.j48");
        Instances attributesStructure = loadFeatureStructure("models/features.arff");
        ...
        for(int i = 0; i < structure.numAttributes(); ++i) {
            if (i == structure.classIndex()) {
                ...
                if (structure.attribute(i).isNumeric()) {
                    ...
                    int index;
                    for(index = structure.attribute(i).indexOfValue(val); index == -1; index = structure.attribute(i).indexOfValue(val)) {
